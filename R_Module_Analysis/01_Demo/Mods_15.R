...
table1 <- demo_table3[,(colnames(wide_table))]

all.equal(table1,wide_table)
...
plt <- ggplot(mpg,aes(y=hwy)) #import dataset into ggplot2
 ...
 plt <- ggplot(mpg,aes(x=manufacturer,y=hwy)) #import dataset into ggplot2
 plt + geom_boxplot(linetype="dotted", fill="lightblue") + theme(axis.text.x=element_text(angle=45,hjust=1),) #add boxplot and rotate x-axis labels 45 degrees
...
summary(aov(hp ~ cyl,data=mtcars_filt)) #see p-values from AnoVa test
